...
final public class Macros {
      ...
      do {
        ...
        for command in commandsArray {
          ...
          if evaluatedCommand.active {
            ...
            // send the command
            _api.radio!.sendCommand(evaluatedCommand.cmd)
        ...
        _log.logMessage("Error reading file \(fileString)", .error, #function, #file, #line)
      }
    }
  ...
  public func evaluateCondition(_ condition: String) -> Bool {
    ...
    guard components.count == 2 else {
      _log.logMessage("Malformed macro condition - \(condition)", .error, #function, #file, #line)
    ...
    if let object = findObject(id: components) {
      ...
      default:
        _log.logMessage("Unknown macro action - \(String(components[1]))", .error, #function, #file, #line)
      }
    }
    return result
  }
  ...
  public func findObject(id: [String.SubSequence]) -> AnyObject? {
    ...
    case "S":                       // Slice
      switch number {
      case "A":
        return Api.sharedInstance.radio!.findActiveSlice()
        
      case "0","1","2","3","4","5","6","7":
        return _api.radio!.slices[String(number).objectId!]
        
      default:
        _log.logMessage("Macro error: slice - \(String(number))", .error, #function, #file, #line)
    ...
    case "P":                       // Panadapter
      ...
      default:
        _log.logMessage("Macro error: panadapter - \(String(number))", .error, #function, #file, #line)
        
        return nil
      }
      
    default:
     _log.logMessage("Macro error: object - \(String(object))", .error, #function, #file, #line)
  ...
  public func findValue(of object: AnyObject, param: String.SubSequence) -> String? {
    ...
    if let slice = object as? xLib6000.Slice {              // Slice params
      ...
      switch p.lowercased() {
      case "id":
        return String(slice.id)
        
      case "freq":
        return (slice.frequency + value).hzToMhz
        
      default:
        _log.logMessage("Macro error: slice param - \(String(p))", .error, #function, #file, #line)
        return nil
      }
      ...
      default:
        _log.logMessage("Macro error: panadapter param - \(String(p))", .error, #function, #file, #line)
        return nil
      }
    }
    return nil
